...
describe('TraceManager', () => {
	...
	it('should not add trace when disabled', () => {
		traceManager.addTrace({msg: 'test'} as any);
		expect(fs.existsSync(testFile)).toBe(false);
	});

	it('should add trace when enabled', () => {
		traceManager.setEnabled(true);
		const entry = {id: '1', timestamp: 'now', source: 'src', url: 'url', method: 'GET', status: '200', duration: 100};
	...
	it('should clear traces', () => {
		...
		traceManager.addTrace({a: 1} as any);
