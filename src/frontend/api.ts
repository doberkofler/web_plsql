...
export const typedApi = {
	 ...
	 * @param includeConfig - Whether to include server configuration.
	 * @returns The validated status response.
	 */
	async getStatus(includeHistory = false, includeConfig = false): Promise<StatusResponse> {
		const res = await fetchWithRetry(`api/status?history=${includeHistory}&config=${includeConfig}`);
