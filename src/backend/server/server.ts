...
export const startServer = async (config: configInputType, ssl?: sslConfig): Promise<webServer> => {
	...
	if (internalConfig.setupRawExtensions) {
		await internalConfig.setupRawExtensions(app);
	}
	...
	for (const i of internalConfig.routeStatic) {
		const middleware =
			i.staticMode === 'dynamic'
				? express.static(i.directoryPath)
				: expressStaticGzip(i.directoryPath, {
						enableBrotli: true,
						orderPreference: ['br'],
					});

		app.use(i.route, middleware);
