...
import {getBinding, getProcedureNamed} from '../../handler/plsql/procedureNamed.ts';
...
describe('handler/plsql/procedureNamed', () => {
	...
	describe('loadArguments', () => {
		...
		it('should throw error when zod parsing fails', async () => {
			...
			await assert.rejects(
				...
				(err) => {
					assert.ok((err as Error).message.includes('Error when decoding arguments'));
					return true;
				},
			);
		});
		...
		it('should throw error when database execute fails', async () => {
			...
			await assert.rejects(
				...
				(err) => {
					assert.ok((err as Error).message.includes('Error when retrieving arguments'));
					return true;
				},
			);
		});
	...
	describe('getProcedureNamed', () => {
		...
		it('should handle unknown argument gracefully (log warning and use default string bind)', async () => {
			...
			assert.strictEqual(bind.p_P2?.type, oracledb.DB_TYPE_VARCHAR);
		});
	});
});
