...
async function invoke(procedure: string, argObj: Object, cgiObj: Object, filesToUpload: filesUploadType, options: oracleExpressMiddleware$options, database: Database): Promise<string> {
	debug(`invoke: procedure="${procedure}"`, argObj, cgiObj, filesToUpload);
...
async function executeProcedure(para: {sql: string, bind: oracledb$bindingType}, cgiObj: Object, options: oracleExpressMiddleware$options, database: Database): Promise<string> {
	...
	let result;
	try {
		result = await database.execute(sql.join('\n'), Object.assign(bind, para.bind));
	} catch (e) {
		const separator = '*'.repeat(40);
		const line = '-'.repeat(10);
		const error = `${separator}\nERROR when trying to execute sql statement:\n${line}\n${sql.join('\n')}\n${line}\n${e.toString()}\n${separator}`;
		debug(error);
	...
	if (result.outBinds.irows > MAX_IROWS) {
		const error = `Unable to retrieve all rows. irows="${result.outBinds.irows}"`;
		debug(error);
