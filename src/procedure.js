...
const debug = require('debug')('web_plsql:procedure');
...
*/
module.exports = async function invoke(req: $Request, res: $Response, argObj: Object, cgiObj: Object, filesToUpload: filesUploadType, options: oracleExpressMiddleware$options, databaseConnection: oracledb$connection): Promise<string> {
	debug(`invoke: procedure="${req.params.name}"`, argObj, cgiObj, filesToUpload);

	const procedure = req.params.name;

	// Upload all files
	debug(`invoke: upload ${filesToUpload.length} files`);
	...
	debug('invoke: executeProcedure');
	return executeProcedure(para, cgiObj, options, databaseConnection);
};
...
async function executeProcedure(para: {sql: string, bind: oracledb$bindingType}, cgiObj: Object, options: oracleExpressMiddleware$options, databaseConnection: oracledb$connection): Promise<string> {
	...
	// combine page
	debug('executeProcedure: join rows');
	const page = result.outBinds.page.join('');

	debug('executeProcedure: end');
