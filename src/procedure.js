...
export const invokeProcedure = async (req, res, argObj, cgiObj, filesToUpload, options, databaseConnection) => {
	...
	const para = await getProcedure(procedure, argObj, options, databaseConnection);
...
const getProcedure = async (procedure, argObj, options, databaseConnection) => {
	if (options.pathAlias && options.pathAlias.alias.toLocaleLowerCase() === procedure.toLocaleLowerCase()) {
		...
		return getProcedureVariable(procedure.substring(1), argObj);
	}

	return await getProcedureNamed(procedure, argObj, databaseConnection);
};
